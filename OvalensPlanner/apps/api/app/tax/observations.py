...
def detect_observations(
    ...
    *,
        ...
        obs.append(
            ObservationItem(
                ...
                savings_breakdown=SavingsBreakdown(
                    current_state=[
                        SavingsBreakdownItem(
                            "Adjusted Net Income", f"£{ani.adjusted_net_income:,.0f}"
                        ),
                        SavingsBreakdownItem(
                            "Personal Allowance", f"£{ani.personal_allowance:,.0f} (tapered)"
                        ),
                    ...
                    cost_note=(
                        f"Net take-home reduces but pension pot grows by £{excess:,.0f} more."
                    ),
                    model_prompt=(
                        f"Model salary sacrifice increase of £{excess:,.0f} "
                        "to restore my personal allowance"
                    ),
                ),
            )
        )
            ...
            ObservationItem(
                ...
                savings_breakdown=SavingsBreakdown(
                    current_state=[
                        ...
                        SavingsBreakdownItem(
                            "Excess above £125,140", f"£{excess_over_restore:,.0f}"
                        ),
                    ],
                    recommended_action=[
                    ...
                    tax_impact=[
                        TaxImpactItem(
                            "Income tax saved (PA restoration)", pa_saving, pa_saving / 12
                        ),
                    ],
                    total_annual=pa_saving,
                    total_monthly=pa_saving / 12,
                    cost_note=(
                        f"Sacrifice £{excess_over_restore:,.0f} more to start restoring PA. "
                        "Full restoration requires ANI ≤ £100,000."
                    ),
                    model_prompt=(
                        f"Model salary sacrifice increase of £{excess_over_restore:,.0f} "
                        "to restore my personal allowance"
                    ),
                ),
            )
            ...
            ObservationItem(
                ...
                savings_breakdown=SavingsBreakdown(
                    current_state=[
                        ...
                        SavingsBreakdownItem(
                            "Child Benefit annual", f"£{hicbc.child_benefit_annual:,.2f}"
                        ),
                    ...
                    recommended_action=[
                        ...
                        SavingsBreakdownItem(
                            "Benefit retained", f"£{hicbc.child_benefit_annual:,.2f}/yr"
                        ),
                    ],
                    tax_impact=[
                        TaxImpactItem(
                            "HICBC charge avoided", hicbc.hicbc_charge, hicbc.hicbc_charge / 12
                        ),
                    ],
                    total_annual=hicbc.hicbc_charge,
                    total_monthly=hicbc.hicbc_charge / 12,
                    cost_note=(
                        "Reduce ANI via pension sacrifice or other deductions "
                        "to eliminate the charge entirely."
                    ),
                    model_prompt=(
                        "Model salary sacrifice to reduce ANI below £60,000 to avoid HICBC"
                    ),
                ),
            )
        )
        ...
        obs.append(
            ObservationItem(
                ...
                action=(
                    ...
                    f"£{potential:,.0f} in tax at your {marginal_rate:.0%} "
                    "marginal rate."
                ),
                savings_breakdown=SavingsBreakdown(
                    current_state=[
                        SavingsBreakdownItem(
                            "Pension AA remaining", f"£{pension_aa.remaining:,.0f}"
                        ),
                        SavingsBreakdownItem(
                            "Current contributions", f"£{pension_contributions:,.0f}"
                        ),
                        SavingsBreakdownItem("Marginal tax rate", f"{marginal_rate:.0%}"),
                    ],
                    recommended_action=[
                        SavingsBreakdownItem(
                            "Max additional contribution", f"£{pension_aa.remaining:,.0f}"
                        ),
                    ...
                    model_prompt=(
                        f"Model increasing pension contributions by £{pension_aa.remaining:,.0f}"
                    ),
                )
                if potential > 0
                else None,
        ...
        obs.append(
            ObservationItem(
                ...
                description=(
                    ...
                    f"£{pension_aa.total_available:,.0f} total available "
                    "allowance. Exceeding this triggers a tax charge."
                ),
                severity="warning",
                category="pension",
            )
        ...
        obs.append(
            ObservationItem(
                ...
                description=(
                    ...
                    "you may be able to transfer £1,260 of unused allowance to a spouse "
                    "or civil partner, saving them up to £252 per year."
                ),
                severity="opportunity",
                category="income_tax",
                potential_saving=252,
                action=(
                    "Check if your spouse/partner is a basic rate taxpayer "
    ...
    savings_income = (
        sum(b.income_in_band for b in income_tax.savings_bands) if income_tax.savings_bands else 0
    )
    ...
    if gift_aid > 0 and marginal > 0.20:
        ...
        obs.append(
            ObservationItem(
                ...
                description=(
                    f"As a {marginal:.0%} rate taxpayer, your £{gift_aid:,.0f} "
                    "Gift Aid donations qualify for additional tax relief of "
                    f"£{extra_relief:,.0f} via your Self Assessment."
                ),
                severity="opportunity",
                category="income_tax",
        ...
        if cgt_gains > aea:
            obs.append(
                ObservationItem(
                    ...
                    description=(
                        f"Your capital gains of £{cgt_gains:,.0f} exceed the "
                        f"£{aea:,} annual exemption. "
                        f"£{cgt_gains - aea:,.0f} is subject to Capital Gains Tax."
                    ),
                    severity="warning",
                    category="capital_gains",
                    action=(
                        "Consider spreading disposals across tax years or "
