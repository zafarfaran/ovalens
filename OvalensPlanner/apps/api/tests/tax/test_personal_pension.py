...
def test_basic_income_tax_saving():
    ...
    sources = [IncomeSource(IncomeType.EMPLOYMENT, 80000, "Employment")]
    r, _ = analyse_personal_pension(sources, proposed_contribution=10000)
    ...
    assert r["proposed"]["pension_contribution"] == 10000
...
def test_pa_taper_restoration():
    ...
    sources = [IncomeSource(IncomeType.EMPLOYMENT, 125140, "Employment")]
    r, _ = analyse_personal_pension(sources, proposed_contribution=25140)

    # Current: ANI=125,140 → PA=0. Proposed: ANI=100,000 → PA=12,570.
    assert r["pa_change"]["current"] == 0
    assert r["pa_change"]["proposed"] == 12570
    assert r["pa_change"]["restored"] == 12570


def test_hicbc_avoidance():
    """£70k salary + 2 children, contribute £10k to bring ANI to £60k → HICBC eliminated."""
    sources = [IncomeSource(IncomeType.EMPLOYMENT, 70000, "Employment")]
    r, _ = analyse_personal_pension(
        sources,
        proposed_contribution=10000,
...
def test_threshold_identification():
    ...
    sources = [IncomeSource(IncomeType.EMPLOYMENT, 120000, "Employment")]
    r, _ = analyse_personal_pension(sources, proposed_contribution=5000)
    ...
    assert pa_thresh[0]["contribution_needed"] == 20000  # 120k - 100k
...
def test_threshold_hicbc_with_children():
    ...
    sources = [IncomeSource(IncomeType.EMPLOYMENT, 75000, "Employment")]
    r, _ = analyse_personal_pension(
        sources,
        proposed_contribution=5000,
        number_of_children=2,
        claims_child_benefit=True,
    )
    ...
    assert hicbc_thresh[0]["contribution_needed"] == 15000  # 75k - 60k


def test_pension_aa_warning():
    """Contribution exceeding AA should generate a warning."""
    sources = [IncomeSource(IncomeType.EMPLOYMENT, 200000, "Employment")]
    r, _ = analyse_personal_pension(sources, proposed_contribution=65000)
...
def test_existing_contribution_increase():
    ...
    r, _ = analyse_personal_pension(
        sources,
        proposed_contribution=15000,
        current_contribution=5000,
    )
    assert r["current"]["pension_contribution"] == 5000
    assert r["proposed"]["pension_contribution"] == 15000
    assert r["savings"]["total"] > 0
...
def test_effective_relief_rate():
    ...
    expected_rate = r["savings"]["total"] / 10000 * 100
...
def test_multiple_income_sources():
    ...
    sources = [
        IncomeSource(IncomeType.EMPLOYMENT, 100000, "Employment"),
        IncomeSource(IncomeType.DIVIDENDS, 20000, "Dividends"),
        IncomeSource(IncomeType.RENTAL, 15000, "Rental"),
    ]
    r, _ = analyse_personal_pension(sources, proposed_contribution=10000)
    assert r["savings"]["total"] > 0
...
def test_net_benefit_higher_rate_taxpayer():
    ...
    nb = r["net_benefit"]
    assert nb["gross_contribution"] == 10000
    assert nb["net_cost_to_client"] == 8000          # 10k * 0.8
    assert nb["basic_rate_relief"] == 2000            # 10k * 0.2
...
def test_net_benefit_basic_rate_taxpayer():
    ...
    sources = [IncomeSource(IncomeType.EMPLOYMENT, 30000, "Employment")]
    r, _ = analyse_personal_pension(sources, proposed_contribution=5000)

    nb = r["net_benefit"]
    assert nb["gross_contribution"] == 5000
    assert nb["net_cost_to_client"] == 4000
    assert nb["basic_rate_relief"] == 1000
    # Basic rate taxpayer: BRB extension doesn't help (already in basic band)
    # So higher_rate_relief should be ~0
    assert nb["higher_rate_relief"] == r["savings"]["income_tax"]
    assert nb["total_tax_relief"] >= 1000  # At least the basic rate relief
...
def test_net_benefit_pa_taper_zone():
    ...
    sources = [IncomeSource(IncomeType.EMPLOYMENT, 110000, "Employment")]
    r, _ = analyse_personal_pension(sources, proposed_contribution=10000)

    nb = r["net_benefit"]
    assert nb["gross_contribution"] == 10000
    assert nb["net_cost_to_client"] == 8000
    assert nb["basic_rate_relief"] == 2000
    # In PA taper zone, IT saving includes PA restoration = very high
    assert nb["higher_rate_relief"] > 3000  # Much more than standard 40% due to PA taper
...
def test_net_benefit_existing_contribution_increase():
    ...
    assert nb["gross_contribution"] == 10000  # 15k - 5k
    assert nb["net_cost_to_client"] == 8000   # 10k * 0.8
    assert nb["basic_rate_relief"] == 2000    # 10k * 0.2
...
def test_net_benefit_additional_rate_taxpayer():
    ...
    sources = [IncomeSource(IncomeType.EMPLOYMENT, 200000, "Employment")]
    r, _ = analyse_personal_pension(sources, proposed_contribution=20000)

    nb = r["net_benefit"]
    assert nb["basic_rate_relief"] == 4000  # 20k * 0.2
...
def test_net_benefit_aa_warning_still_fires():
    ...
    assert r["net_benefit"]["gross_contribution"] == 65000
...
def test_total_benefit_zero_contribution():
    ...
    sources = [IncomeSource(IncomeType.EMPLOYMENT, 80000, "Employment")]
    r, _ = analyse_personal_pension(
        sources, proposed_contribution=5000, current_contribution=5000
    )
