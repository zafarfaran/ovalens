...
def test_salary_sacrifice_net_benefit():
    ...
    nb = r["net_benefit"]
    assert nb["gross_into_pension"] == 20000
    ...
    assert nb["take_home_reduction"] == 20000 - r["savings"]["total"]
...
def test_salary_sacrifice_net_benefit_existing_sacrifice():
    ...
    r, _ = analyse_salary_sacrifice(100000, 15000, current_sacrifice=5000)

    nb = r["net_benefit"]
    # Net benefit is on the ADDITIONAL sacrifice
    assert nb["gross_into_pension"] == 10000  # 15k - 5k
    assert nb["total_saving"] == r["savings"]["total"]
    assert nb["take_home_reduction"] == 10000 - nb["total_saving"]
...
def test_total_benefit_employer_ni_amount():
    ...
    # Difference should be approximately 20000 * 0.15 = £3,000
    assert 2500 < tb["employer_ni_saved"] < 3500
...
def test_total_benefit_salary_sacrifice_zero():
    ...
    r, _ = analyse_salary_sacrifice(100000, 5000, current_sacrifice=5000)
