...
def test_pa_exactly_at_lost_boundary():
    ...
    r = calculate_adjusted_net_income(125140)
    assert r.personal_allowance == 0
    assert r.pa_status == PAStatus.LOST
...
def test_pension_reduces_ani():
    ...
    r = calculate_adjusted_net_income(112000, pension_contributions=12000)
    assert r.adjusted_net_income == 100000
    assert r.personal_allowance == 12570
    assert r.pa_status == PAStatus.FULL
...
def test_gift_aid_grossed_up():
    ...
    r = calculate_adjusted_net_income(110000, gift_aid=8000)
    assert r.gift_aid_gross == 10000
    assert r.adjusted_net_income == 100000
    assert r.personal_allowance == 12570
    assert r.pa_status == PAStatus.FULL
...
def test_sarah_mitchell():
    ...
    r = calculate_adjusted_net_income(195500, pension_contributions=18000)
    assert r.adjusted_net_income == 177500
    assert r.personal_allowance == 0
    assert r.pa_status == PAStatus.LOST
